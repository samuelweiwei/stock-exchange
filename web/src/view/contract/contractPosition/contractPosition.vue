  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="日期" prop="createdAt" width="180">
          ...
          <el-table-column align="left" label="所在用户表的用户 ID" prop="userId" width="120" />
         ...
         <el-table-column align="left" label="持仓时间" prop="positionTime" width="180">
            <template #default="scope">{{ formatDate(scope.row.positionTime) }}</template>
         </el-table-column>
          <el-table-column align="left" label="持仓数量" prop="quantity" width="120" />
          ...
          <el-table-column align="left" label="保证金" prop="margin" width="120" />
          <el-table-column align="left" label="持仓金额" prop="positionAmount" width="120" />
          <el-table-column align="left" label="强平价格" prop="forceClosePrice" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateContractPositionFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="所在用户表的用户 ID:"  prop="userId" >
              <el-input v-model.number="formData.userId" :clearable="true" placeholder="请输入所在用户表的用户 ID" />
            </el-form-item>
            ...
            <el-form-item label="持仓时间:"  prop="positionTime" >
              <el-date-picker v-model="formData.positionTime" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
            </el-form-item>
            ...
            <el-form-item label="保证金:"  prop="margin" >
              <el-input-number v-model="formData.margin"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
            <el-form-item label="持仓金额:"  prop="positionAmount" >
              <el-input-number v-model="formData.positionAmount"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
            <el-form-item label="强平价格:"  prop="forceClosePrice" >
              <el-input-number v-model="formData.forceClosePrice"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    <el-descriptions-item label="所在用户表的用户 ID">
                        {{ detailFrom.userId }}
                    ...
                    <el-descriptions-item label="持仓金额">
                        {{ detailFrom.positionAmount }}
                    </el-descriptions-item>
                    <el-descriptions-item label="强平价格">
                        {{ detailFrom.forceClosePrice }}
                    </el-descriptions-item>
...
import {
  ...
  deleteContractPositionByIds,
  updateContractPosition,
  findContractPosition,
  getContractPositionList
...
const getTableData = async() => {
  const table = await getContractPositionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteContractPositionFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteContractPositionByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateContractPositionFunc = async(row) => {
    const res = await findContractPosition({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteContractPositionFunc = async (row) => {
    const res = await deleteContractPosition({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
