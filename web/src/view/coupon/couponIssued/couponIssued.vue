  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
          ...
          <el-table-column align="left" label="优惠券类型：0-未知券 1-注册送券 2-实名送券 3-人工送券" prop="type" width="120" />
        ...
        <el-table-column align="left" label="0未使用 1已使用" prop="status" width="120">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateCouponIssuedFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="优惠券类型：0-未知券 1-注册送券 2-实名送券 3-人工送券:"  prop="type" >
              <el-input v-model.number="formData.type" :clearable="true" placeholder="请输入优惠券类型：0-未知券 1-注册送券 2-实名送券 3-人工送券" />
            </el-form-item>
            ...
            <el-form-item label="0未使用 1已使用:"  prop="status" >
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="优惠券类型：0-未知券 1-注册送券 2-实名送券 3-人工送券">
                        {{ detailFrom.type }}
                    </el-descriptions-item>
                    ...
                    <el-descriptions-item label="0未使用 1已使用">
                        {{ detailFrom.status }}
                    </el-descriptions-item>
...
import {
  createCouponIssued,
  deleteCouponIssued,
  deleteCouponIssuedByIds,
  updateCouponIssued,
  findCouponIssued,
  getCouponIssuedList
} from '@/api/coupon/couponIssued'
...
const getTableData = async() => {
  const table = await getCouponIssuedList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCouponIssuedByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCouponIssuedFunc = async(row) => {
    const res = await findCouponIssued({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCouponIssuedFunc = async (row) => {
    const res = await deleteCouponIssued({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCouponIssued(formData.value)
                  break
                case 'update':
                  res = await updateCouponIssued(formData.value)
                  break
                default:
                  res = await createCouponIssued(formData.value)
                  break
