...
<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="优惠券类型：0-注册送券" prop="type" width="120">
          ...
          <el-table-column align="left" label="优惠金额" prop="amount" width="120" />
        <el-table-column align="left" label="启用状态: 0-关闭" prop="activeStatus" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.activeStatus) }}</template>
        </el-table-column>
          <el-table-column align="left" label="有效时长,单位天,只适用于注册券和实名券" prop="validDays" width="120" />
          <el-table-column align="left" label="有效期开始时间，只适用于手动发券" prop="validStart" width="120" />
          <el-table-column align="left" label="有效期结束时间，只适用于手动发券" prop="validEnd" width="120" />
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="优惠券类型：0-注册送券:"  prop="type" >
            ...
            <el-form-item label="启用状态: 0-关闭:"  prop="activeStatus" >
              <el-switch v-model="formData.activeStatus" active-color="#13ce66" inactive-color="#ff4949" active-text="是" inactive-text="否" clearable ></el-switch>
            </el-form-item>
            <el-form-item label="有效时长,单位天,只适用于注册券和实名券:"  prop="validDays" >
              <el-input v-model.number="formData.validDays" :clearable="true" placeholder="请输入有效时长,单位天,只适用于注册券和实名券" />
            </el-form-item>
            <el-form-item label="有效期开始时间，只适用于手动发券:"  prop="validStart" >
              <el-input v-model.number="formData.validStart" :clearable="true" placeholder="请输入有效期开始时间，只适用于手动发券" />
            </el-form-item>
            <el-form-item label="有效期结束时间，只适用于手动发券:"  prop="validEnd" >
              <el-input v-model.number="formData.validEnd" :clearable="true" placeholder="请输入有效期结束时间，只适用于手动发券" />
            </el-form-item>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="优惠券类型：0-注册送券">
                        {{ detailFrom.type }}
                    </el-descriptions-item>
                    ...
                    <el-descriptions-item label="启用状态: 0-关闭">
                        {{ detailFrom.activeStatus }}
                    </el-descriptions-item>
                    <el-descriptions-item label="有效时长,单位天,只适用于注册券和实名券">
                        {{ detailFrom.validDays }}
                    </el-descriptions-item>
                    <el-descriptions-item label="有效期开始时间，只适用于手动发券">
                        {{ detailFrom.validStart }}
                    </el-descriptions-item>
                    <el-descriptions-item label="有效期结束时间，只适用于手动发券">
                        {{ detailFrom.validEnd }}
                    </el-descriptions-item>
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    if (searchInfo.value.activeStatus === ""){
        searchInfo.value.activeStatus=null
    }
    getTableData()
  })
}
