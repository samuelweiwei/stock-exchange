...
<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="状态|||1: 质押,2:赎回" prop="status" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.status) }}</template>
        </el-table-column>
        <el-table-column align="left" label="1提前赎回, 2未提前赎回" prop="redeemInAdvance" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.redeemInAdvance) }}</template>
        </el-table-column>
          <el-table-column align="left" label="罚金" prop="fine" width="120" />
          <el-table-column align="left" label="活/定期产品开始时间" prop="startAt" width="120" />
          <el-table-column align="left" label="活/定期产品到期时间" prop="endAt" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateEarnSubscribeLogFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="罚金:"  prop="fine" >
              <el-input-number v-model="formData.fine"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
            <el-form-item label="活/定期产品开始时间:"  prop="startAt" >
              <el-input v-model.number="formData.startAt" :clearable="true" placeholder="请输入活/定期产品开始时间" />
            </el-form-item>
            <el-form-item label="活/定期产品到期时间:"  prop="endAt" >
              <el-input v-model.number="formData.endAt" :clearable="true" placeholder="请输入活/定期产品到期时间" />
            </el-form-item>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="状态|||1: 质押,2:赎回">
                        {{ detailFrom.status }}
                    </el-descriptions-item>
                    <el-descriptions-item label="1提前赎回, 2未提前赎回">
                        {{ detailFrom.redeemInAdvance }}
                    </el-descriptions-item>
                    <el-descriptions-item label="罚金">
                        {{ detailFrom.fine }}
                    </el-descriptions-item>
                    <el-descriptions-item label="活/定期产品开始时间">
                        {{ detailFrom.startAt }}
                    </el-descriptions-item>
                    <el-descriptions-item label="活/定期产品到期时间">
                        {{ detailFrom.endAt }}
                    </el-descriptions-item>
...
import {
  ...
  deleteEarnSubscribeLog,
  deleteEarnSubscribeLogByIds,
  updateEarnSubscribeLog,
  findEarnSubscribeLog,
  getEarnSubscribeLogList
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    if (searchInfo.value.redeemInAdvance === ""){
        searchInfo.value.redeemInAdvance=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getEarnSubscribeLogList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteEarnSubscribeLogFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteEarnSubscribeLogByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateEarnSubscribeLogFunc = async(row) => {
    const res = await findEarnSubscribeLog({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteEarnSubscribeLogFunc = async (row) => {
    const res = await deleteEarnSubscribeLog({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
