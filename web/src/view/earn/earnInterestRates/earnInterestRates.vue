  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
          ...
          <el-table-column align="left" label="利率" prop="interestRates" width="120" />
         <el-table-column align="left" label="period字段" prop="period" width="180">
            <template #default="scope">{{ formatDate(scope.row.period) }}</template>
         </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateEarnInterestRatesFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="period字段:"  prop="period" >
              <el-date-picker v-model="formData.period" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
            </el-form-item>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="period字段">
                        {{ detailFrom.period }}
                    </el-descriptions-item>
...
import {
  ...
  deleteEarnInterestRates,
  deleteEarnInterestRatesByIds,
  updateEarnInterestRates,
  findEarnInterestRates,
  getEarnInterestRatesList
...
const getTableData = async() => {
  const table = await getEarnInterestRatesList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteEarnInterestRatesFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteEarnInterestRatesByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateEarnInterestRatesFunc = async(row) => {
    const res = await findEarnInterestRates({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteEarnInterestRatesFunc = async (row) => {
    const res = await deleteEarnInterestRates({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
