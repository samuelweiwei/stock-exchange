  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
          ...
          <el-table-column align="left" label="父分类ID" prop="parentCategoryId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateSysI18nDictCategoryFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="分类ID:"  prop="id" >
              <el-input v-model.number="formData.id" :clearable="true" placeholder="请输入分类ID" />
            </el-form-item>
            <el-form-item label="父分类ID:"  prop="parentCategoryId" >
              <el-input v-model.number="formData.parentCategoryId" :clearable="true" placeholder="请输入父分类ID" />
            </el-form-item>
...
import {
  ...
  deleteSysI18nDictCategory,
  deleteSysI18nDictCategoryByIds,
  updateSysI18nDictCategory,
  findSysI18nDictCategory,
  getSysI18nDictCategoryList
...
const getTableData = async() => {
  const table = await getSysI18nDictCategoryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteSysI18nDictCategoryFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSysI18nDictCategoryByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSysI18nDictCategoryFunc = async(row) => {
    const res = await findSysI18nDictCategory({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSysI18nDictCategoryFunc = async (row) => {
    const res = await deleteSysI18nDictCategory({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
