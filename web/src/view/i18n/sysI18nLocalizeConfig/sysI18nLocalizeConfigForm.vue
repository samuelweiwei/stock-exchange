    ...
    <div class="gva-form-box">
      <el-form :model="formData" ref="elFormRef" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="错误码:" prop="errorCode">
          <el-input v-model.number="formData.errorCode" :clearable="true" placeholder="请输入" />
       </el-form-item>
...
defineOptions({
    name: 'SysI18nLocalizeConfigForm'
})
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findSysI18nLocalizeConfig({ ID: route.query.id })
      if (res.code === 0) {
