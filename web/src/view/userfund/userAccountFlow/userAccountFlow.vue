  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="日期" prop="createdAt" width="180">
          ...
          <el-table-column align="left" label="1 充值 2 提现 3 划转到合约账户 4 合约账户划转进来" prop="transactionType" width="120" />
          <el-table-column align="left" label="amount字段" prop="amount" width="120" />
          <el-table-column align="left" label="balanceAfter字段" prop="balanceAfter" width="120" />
         <el-table-column align="left" label="transactionDate字段" prop="transactionDate" width="180">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserAccountFlowFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="balanceAfter字段:"  prop="balanceAfter" >
              <el-input-number v-model="formData.balanceAfter"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
            <el-form-item label="transactionDate字段:"  prop="transactionDate" >
              <el-date-picker v-model="formData.transactionDate" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
            </el-form-item>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="1 充值 2 提现 3 划转到合约账户 4 合约账户划转进来">
                        {{ detailFrom.transactionType }}
                    </el-descriptions-item>
                    <el-descriptions-item label="amount字段">
                        {{ detailFrom.amount }}
                    </el-descriptions-item>
                    <el-descriptions-item label="balanceAfter字段">
                        {{ detailFrom.balanceAfter }}
                    </el-descriptions-item>
                    <el-descriptions-item label="transactionDate字段">
                        {{ detailFrom.transactionDate }}
                    </el-descriptions-item>
...
import {
  ...
  deleteUserAccountFlowByIds,
  updateUserAccountFlow,
  findUserAccountFlow,
  getUserAccountFlowList
...
const getTableData = async() => {
  const table = await getUserAccountFlowList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserAccountFlowByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserAccountFlowFunc = async(row) => {
    const res = await findUserAccountFlow({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserAccountFlowFunc = async (row) => {
    const res = await deleteUserAccountFlow({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
