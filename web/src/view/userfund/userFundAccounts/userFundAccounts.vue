  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
          ...
          <el-table-column align="left" label="assetType字段" prop="assetType" width="120" />
          <el-table-column align="left" label="balance字段" prop="balance" width="120" />
          <el-table-column align="left" label="frozenBalance字段" prop="frozenBalance" width="120" />
          <el-table-column align="left" label="availableBalance字段" prop="availableBalance" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserFundAccountsFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="frozenBalance字段:"  prop="frozenBalance" >
              <el-input-number v-model="formData.frozenBalance"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
            <el-form-item label="availableBalance字段:"  prop="availableBalance" >
              <el-input-number v-model="formData.availableBalance"  style="width:100%" :precision="2" :clearable="true"  />
            </el-form-item>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    ...
                    <el-descriptions-item label="assetType字段">
                        {{ detailFrom.assetType }}
                    </el-descriptions-item>
                    <el-descriptions-item label="balance字段">
                        {{ detailFrom.balance }}
                    </el-descriptions-item>
                    <el-descriptions-item label="frozenBalance字段">
                        {{ detailFrom.frozenBalance }}
                    </el-descriptions-item>
                    <el-descriptions-item label="availableBalance字段">
                        {{ detailFrom.availableBalance }}
                    </el-descriptions-item>
...
import {
  ...
  deleteUserFundAccounts,
  deleteUserFundAccountsByIds,
  updateUserFundAccounts,
  findUserFundAccounts,
  getUserFundAccountsList
...
const getTableData = async() => {
  const table = await getUserFundAccountsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserFundAccountsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserFundAccountsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserFundAccountsFunc = async(row) => {
    const res = await findUserFundAccounts({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserFundAccountsFunc = async (row) => {
    const res = await deleteUserFundAccounts({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
