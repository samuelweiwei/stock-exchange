    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
         ...
         <el-table-column align="left" label="deletedAt字段" prop="deletedAt" width="180">
          ...
          <el-table-column align="left" label="手机区号" prop="code" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateNationalityInfoFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="手机区号:"  prop="code" >
              <el-input v-model="formData.code" :clearable="true"  placeholder="请输入手机区号" />
            </el-form-item>
            <el-form-item label="状态: 1 开启, 0 未开启:"  prop="status" >
              <el-input v-model.number="formData.status" :clearable="true" placeholder="请输入状态: 1 开启, 0 未开启" />
            </el-form-item>
          </el-form>
    </el-drawer>
...
<script setup>
import {
  createNationalityInfo,
  deleteNationalityInfo,
  deleteNationalityInfoByIds,
  updateNationalityInfo,
  findNationalityInfo,
  getNationalityInfoList
} from '@/api/user/nationalityInfo'
...
const getTableData = async() => {
  const table = await getNationalityInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteNationalityInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateNationalityInfoFunc = async(row) => {
    const res = await findNationalityInfo({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteNationalityInfoFunc = async (row) => {
    const res = await deleteNationalityInfo({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createNationalityInfo(formData.value)
                  break
                case 'update':
                  res = await updateNationalityInfo(formData.value)
                  break
                default:
                  res = await createNationalityInfo(formData.value)
                  break
