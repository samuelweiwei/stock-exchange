...
func (b *BaseApi) Login(c *gin.Context) {
	var (
		...
		key          = c.ClientIP()
		systemConfig *systemRes.SystemConfig
	)
	if err != nil {
		_ = c.Error(err)
		return
	...
	//验证ip是否允许登录
	systemConfig, err = systemConfigService.GetPlatformSystemConfig()
	if len(systemConfig.BackWhiteIpStrings) > 0 && !strings.Contains(systemConfig.BackWhiteIpStrings, key) {
		response.FailWithMessage(fmt.Sprintf("此ip:%s 不在登录白名单内", key), c)
		return
	}
...
func (b *BaseApi) Register(c *gin.Context) {
	...
	var (
		user       = &system.SysUser{Username: r.Username, NickName: r.NickName, Password: r.Password, HeaderImg: r.HeaderImg, AuthorityId: r.AuthorityId, Authorities: authorities, Enable: r.Enable, Phone: r.Phone, Email: r.Email, FrontUserid: cast.ToUint(r.FrontUserid)}
		userReturn system.SysUser
	)
...
func (b *BaseApi) SetUserInfo(c *gin.Context) {
	...
	err = userService.SetUserInfo(system.SysUser{
		...
		Email:       user.Email,
		Enable:      user.Enable,
		FrontUserid: cast.ToUint(user.FrontUserid),
	})
	if err != nil {
		global.GVA_LOG.Error("设置失败!", zap.Error(err))
		response.FailWithMessage("设置失败", c)
